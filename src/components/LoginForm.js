...
const LoginForm = ({ setLoggedInUser }) => {
  // Define userId and setUserId using useState
  ...
  const handleLogin = () => {
    // Mock login validation (You can connect to backend API here)
    if (userId && password && role) {
      setLoggedInUser({ role, userId });
      if (role === 'User') {
...
const LoginForm = ({ setLoggedInUser }) => {
  ...
  const handleLogin = () => {
    ...
    if (username && password && name && email && role) {
      setLoggedInUser({ role, username, name, email });
      if (role === 'User') {
        navigate('/grievance-form');
      } else {
        navigate('/grievances');
